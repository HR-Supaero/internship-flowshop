...
def check_robustness(
    ...
    max_deviations = reorder_sequence(job_sequence, max_deviations)

    # For the given sequence, check for all possible scenarios 
    ...
    for scen in scenariis:
        ...
        makespan = flowshop.nowait_cmax(processing_times)
    ...
    for seq in other_sequences:
        ...
        for scen in scenariis:
            ...
            processing_times = calculate_processing_times(scen, nom_processing_times, max_deviations)
            makespan = flowshop.nowait_cmax(processing_times)
            if makespan >= worst_makespan_for_seq:
                worst_makespan_for_seq = makespan
            if makespan > worst_makespan:
                worst_makespan_for_seq = makespan
                #print(f'For the sequence {seq} the scenario \n {scen} \n leads to a makespan of {worst_makespan_for_seq}')
                break
        if worst_makespan_for_seq < worst_makespan:
            #raise ValueError("The sequence {seq} leads to a lower makespan than {worst_makespan}", seq, worst_makespan)
            return False
    return True
...
def check_criticalness(
    ...
    nb_machines = len(nom_processing_times)
    nb_jobs = len(nom_processing_times[0])
    print(f'given nominal processing times \n {nom_processing_times}')
    ...
    for scen in scenariis:
        scen = reorder_sequence(job_sequence, scen)
        proceessing_times = calculate_processing_times(scen, nom_processing_times, max_deviations)
        Cmax = nowait_setup_flowshop.cmax(proceessing_times)
        print(f'The makespan is {Cmax} for scenario \n {scen}')
...
def CP_adverse_problem(job_sequence, nom_processing_times, max_dev, setup_times, budget):
    ...
    #flowshopGantt.ganttCP_setup(var_sol, setup_mat, objective_SP)
